...
The local Perry compiler observed for this plan is 0.5.1022. Before implementing or publishing this API, a minimal fixture must validate, on every project game target, class declarations, constructors, extends, super, method overrides, constructor arguments passed through methods, instanceof including multi-level inheritance, generic component constructor APIs, readonly properties, subclass-preserving return types, and arrays containing subclass instances. Include explicit override keyword support as a separate check; if the keyword fails but method overriding works, examples omit the keyword. Class/instanceof/generic API failures block implementation until the public contract is revised from compiler evidence.
