...
- A component attached to an awake object gets onAwake immediately when no awake traversal is active. Components attached during an awake traversal join that traversal in attachment order. A component attached before its owner enters a scene gets onAwake when that object enters the scene.
...
Objects/components added during a phase receive required onAwake immediately, but are absent from that phase's snapshot and begin update/fixedUpdate callbacks on the next corresponding phase call. Objects/components added during the initial onAwake traversal are incorporated in attachment order before the outer scene attachment returns. Removals/detaches/destroys immediately mark the target ineligible, so a snapshot entry later in the same phase is skipped. Activation and enabled flags are rechecked before every callback.
