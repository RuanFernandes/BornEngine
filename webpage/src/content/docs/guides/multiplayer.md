...
Room joins are asynchronous. In the regular engine loop, `beginDrawing()` pumps Colyseus callbacks automatically, so the `joining` promise resolves as frames run. If a host uses a custom loop, call `client.poll()` while matchmaking and room events are pending.
...
See the [platform networking notes](../../api/colyseus/#verified-platform-matrix), [mobile guide](../../platforms/mobile/), and [Apple guide](../../platforms/apple/) before building for a device.
...
```ts
import {
  beginDrawing, clearBackground, endDrawing, initWindow,
  isKeyDown, Key, runGame,
} from '@bornengine/engine/core';
import { drawRect } from '@bornengine/engine/shapes';
import { drawText } from '@bornengine/engine/text';
import { ColyseusClient, type ColyseusRoom } from '@bornengine/engine/colyseus';
...
initWindow(960, 540, 'Online Arena');
...
client.joinOrCreate<ArenaSnapshot>('arena', { name: 'Player One' }).then(
  (joined) => {
    room = joined;
    snapshot = joined.state;
    status = 'Connected';
    joined.onStateChange((state) => { snapshot = state; });
    joined.onDrop((code, reason) => { status = `Reconnecting (${code}): ${reason}`; });
    joined.onReconnect(() => {
      status = 'Reconnected';
      lastInputX = 2;
      lastInputY = 2;
    });
    joined.onError((error) => { status = error.message; });
    joined.onLeave((code, reason) => { status = `Left room (${code}): ${reason}`; });
  },
  (error) => { status = `Join failed: ${error.message}`; },
);
...
runGame(() => {
  if (room !== null && room.isConnected) {
    let inputX = 0;
    let inputY = 0;
    if (isKeyDown(Key.LEFT)) inputX -= 1;
    if (isKeyDown(Key.RIGHT)) inputX += 1;
    if (isKeyDown(Key.UP)) inputY -= 1;
    if (isKeyDown(Key.DOWN)) inputY += 1;
  ...
  beginDrawing();
  clearBackground({ r: 18, g: 24, b: 34, a: 255 });
  drawText(status, 24, 20, 18, { r: 255, g: 255, b: 255, a: 255 });

  if (snapshot !== null) {
    for (const sessionId of Object.keys(snapshot.players)) {
      const player = snapshot.players[sessionId];
      const isLocal = room !== null && sessionId === room.sessionId;
      const color = isLocal
        ? { r: 184, g: 242, b: 61, a: 255 }
        : { r: 90, g: 170, b: 255, a: 255 };
      drawRect(player.x - 16, player.y - 16, 32, 32, color);
      drawText(player.name, player.x - 28, player.y - 38, 16, { r: 255, g: 255, b: 255, a: 255 });
    }
  }
