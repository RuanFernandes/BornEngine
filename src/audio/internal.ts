...
export async function stageSoundAsync(path: string): Promise<number> {
  return await spawn(() => bloom_stage_sound(path as any));
}

export async function stageMusicAsync(path: string): Promise<number> {
  return await spawn(() => bloom_stage_sound(path as any));
}
