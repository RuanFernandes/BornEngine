...
fn collect_events_for(timeout: Duration) -> Vec<Value> {
    let deadline = Instant::now() + timeout;
        ...
        loop {
            ...
            events.push(
                serde_json::from_str(&encoded).unwrap_or_else(|error| {
                    panic!("invalid Colyseus event JSON: {error}: {encoded}")
                }),
            );
        }
        std::thread::sleep(Duration::from_millis(5));
...
fn native_sdk_matches_the_repository_fixture_contract() {
    ...
    let client = Client(colyseus::client_create(&endpoint));
    ...
    let cleanup_client = Client(colyseus::client_create(&endpoint));
    let cleanup_room = colyseus::client_join(
        cleanup_client.0,
        0,
        "test_room",
        r#"{"name":"cleanup-check"}"#,
    );
    assert_ne!(
        cleanup_room, 0,
        "native SDK should start the cleanup-check join"
    );
    wait_for("cleanup-check join", Duration::from_secs(10), |event| {
        event["kind"] == "join" && event["room"] == cleanup_room
    });
    let pending_request =
        colyseus::room_request(cleanup_room, "request_delay", r#"{"delayMs":1200}"#);
    assert_ne!(
        pending_request, 0,
        "native SDK should start a delayed request"
    );
    colyseus::client_dispose(cleanup_client.0);
    let late_events = collect_events_for(Duration::from_millis(1500));
    assert!(
        !late_events
            .iter()
            .any(|event| event["kind"] == "request" && event["request"] == pending_request),
        "disposing a client should cancel its in-flight request callback: {late_events:?}"
    );

    let pending_client = Client(colyseus::client_create(&endpoint));
    let pending_join = colyseus::client_join(pending_client.0, 0, "test_room", "{}");
    assert_ne!(
        pending_join, 0,
        "native SDK should return a handle for a pending join"
    );
    colyseus::client_dispose(pending_client.0);
    let pending_events = collect_events_for(Duration::from_millis(600));
    assert!(
        !pending_events
            .iter()
            .any(|event| event["room"] == pending_join && event["kind"] == "join"),
        "disposing a client should prevent a late pending-join callback: {pending_events:?}"
    );
    println!("COLYSEUS_NATIVE_SMOKE_OK");
}
